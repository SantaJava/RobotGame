...
//두물체간 충돌 여부 판단 
	...
	var result3=(me_x <= target_x) && (me_x+me_width >= target_x);  //나의 가로폭이 타겟의 가로폭 내에 있는지 판단
	
	var result4=(me_y >= target_y) && (me_y <= (target_y+target_height));//나의 y좌표위치가 타겟의 세로폭 내에 있는지 판단
	var result5=(me_y+me_height >= target_y) && (me_y+me_height <= (target_y+target_height));//나의 y폭이 타겟의 세로폭 내에 있는지 판단
	var result6=(me_y <= target_y) && (me_y+me_height >= target_y);//나의 y폭이 타겟의 세로폭 내에 있는지 판단
